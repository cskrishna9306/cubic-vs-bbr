#!/bin/bash
# Create a new server on another instance with iperf3 -s -p 5202
# Use this script using the following command on the client:
# ./script-bt-results-2.sh $CCA-20ms-delay-f2.txt & iperf3 -c SERVER_IP -p SERVER_PORT --cport CLIENT_PORT_2 -t $TIME -i 0.1 & wait
